...
class SnarxPaperless extends Contract {
  ...
  async instantiate(ctx) {
    ...
    await ctx.stub.putState(
      "customers",
      Buffer.from(JSON.stringify(emptyList))
    );
    await ctx.stub.putState(
      "providers",
      Buffer.from(JSON.stringify(emptyList))
  ...
  async VerifiedUserCustomer(ctx, mailId, userName, companyName, rol) {
    // verify Customer
    let customerData = await ctx.stub.getState(mailId);
    let customer;
    if (customerData) {
      customer = JSON.parse(customerData.toString());
      if (
        (customer.mailId != mailId || customer.userName !== userName,
        customer.companyName !== companyName || customer.rol !== rol)
      ) {
        throw new Error(
          "Customer No Identificado en la Red con los atributos enviados"
        );
      }
    } else {
      throw new Error("No se encontro al Customer");
    }

    return JSON.stringify(customerData);
  }

  // add a customer object to the blockchain state identifited by the mailId
  async RegisterCustomer(
    ...
    await ctx.stub.putState(mailId, Buffer.from(JSON.stringify(customer)));

    //add buyerId to 'buyers' key
    let data = await ctx.stub.getState("customers");
    if (data) {
      let customers = JSON.parse(data.toString());
      customers.push(mailId);
      await ctx.stub.putState(
        "customers",
        Buffer.from(JSON.stringify(customers))
      );
    } else {
      throw new Error("No hay Customers cargados");
    }
  ...
  async VerifiedUserProvider(ctx, mailId) {
    // verify Provider
    let providerData = await ctx.stub.getState(mailId);
    let provider;
    if (providerData) {
      provider = JSON.parse(providerData.toString());
      if (provider.type !== "provider") {
        throw new Error("Provider No Identificado en la Red");
      }
    } else {
      throw new Error("No se encontro al Customer");
    }
  ...
  async RegisterProvider(ctx, mailId, companyName) {
    let provider = {
      emailId: mailId,
      companyName: companyName,
      type: "provider",
    ...
    await ctx.stub.putState(mailId, Buffer.from(JSON.stringify(provider)));

    //add providerId to 'providers' key
    let data = await ctx.stub.getState("providers");
    if (data) {
      let providers = JSON.parse(data.toString());
      providers.push(mailId);
      await ctx.stub.putState(
        "providers",
        Buffer.from(JSON.stringify(providers))
      );
    } else {
